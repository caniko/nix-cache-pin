    ...
    perSystem = {
      ...
      in
        pkgs.writeShellScriptBin "cache-pin-source-pins-${name}" ''
          ...
          # Parse Cargo.lock git sources and prefetch each one
          > "$output_file.tmp"
          echo "# Generated by nix-cache-pin source-pins — do not edit manually." > "$output_file.tmp"
          echo "# Lock file:   $lock" >> "$output_file.tmp"
          echo "# Source pin:  $n" >> "$output_file.tmp"
          echo "# Updated:     $(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$output_file.tmp"
          ...
          rg 'source = "git\+' "$lock" \
            ...
            | while IFS= read -r source; do
                echo "  Prefetching: $source" >&2

                # Convert Cargo.lock source URL to Nix flake URI
                url="${source#git+}"

                if echo "$url" | grep -q '?rev='; then
                  # Has ?rev=... query param — use it directly
                  rev=$(echo "$url" | sed 's/.*?rev=\([^&#]*\).*/\1/')
                  base=$(echo "$url" | sed 's/?rev=[^&#]*//;s/#.*//')
                  fetch_url="${base}?rev=${rev}"
                else
                  # Frag-only: extract rev from fragment
                  rev=$(echo "$url" | sed 's/.*#//')
                  base=$(echo "$url" | sed 's/#.*//')
                  fetch_url="${base}?rev=${rev}"
                fi
                ...
                prefetch_out=$(${nixBin} flake prefetch "$fetch_url" 2>&1) || {
                  echo "    ERROR: prefetch failed for $source" >&2
                  exit 1
                }

                hash=$(echo "$prefetch_out" | grep -oP "hash '\K[^']+")
                echo "    Hash: $hash" >&2

                short=$(echo "$source" | head -c 80)
                echo "  # $short" >> "$output_file.tmp"
                echo "  \"$source\" = \"$hash\";" >> "$output_file.tmp"
              done
